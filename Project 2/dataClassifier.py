...
def isLegal(x, y):
    '''
    Check if the coordinates are legal (within the picture grid).
    '''
    return x < DIGIT_DATUM_WIDTH and (x > 0 or x == 0) and (y > 0 or y==0) and y < DIGIT_DATUM_HEIGHT


def explore(visited, currentPos, datum):
    '''
    Explore all non-highlighted, connected pixels.
    '''
    x = currentPos[0] 
    y = currentPos[1]
    
    visited.append(currentPos)
    # If we can explore a neighbouring pixel, it's non-highlighted and we haven't
    # visited it.
    if  isLegal(x-1, y) and not datum.getPixel(x-1, y) != 0 and [x-1, y] not in visited:
        explore(visited, [x-1, y], datum)
    if  isLegal(x+1, y) and not datum.getPixel(x+1, y) != 0 and [x+1, y] not in visited:
        explore(visited, [x+1, y], datum)
    if  isLegal(x, y-1) and not datum.getPixel(x, y-1) != 0 and [x, y-1] not in visited:
        explore(visited, [x, y-1], datum)
    if  isLegal(x, y+1) and not datum.getPixel(x, y+1) != 0 and [x, y+1] not in visited:
        explore(visited, [x, y+1], datum)


def contSpaces(datum):
    '''
    Visit all non-highlighted pixels in the grid to see how many
    continuous spaces there are.
    '''
    visited = []
    continuousSpaces = 0
    # Visit all non-highlighted pixels and see how many loop iterations it takes
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if not datum.getPixel(x, y) != 0 and [x, y] not in visited:
                continuousSpaces += 1
                explore(visited, [x, y], datum)
    return continuousSpaces
...
def enhancedFeatureExtractorDigit(datum):
    ...
    ## DESCRIBE YOUR ENHANCED FEATURES HERE...
            "Continuous Spaces"
            Counts the amout of continuous spaces in a picture. 
            Continuous space = non-highlighted pixels that are connected 
            together through non-highlighted pixels.
            Generally digits (1, 2, 3, 5, 7) have only 1 continuous space, 
            digits (4, 6, 8, 0) have 2 continuous spaces,
            and digit (8) has 3 cont. spaces.     
    ##
    """
    features =  basicFeatureExtractorDigit(datum)

    cSpaces = contSpaces(datum)
    for i in range(1, 6):
        if i != cSpaces:
            features["continuous_spaces" + str(i)] = 0
        elif i == cSpaces:
            features["continuous_spaces" + str(i)] = 1
    return features
...
def enhancedPacmanFeatures(state, action):
    ...
    foods = list(state.getFood())

    for food in foods:
        foodDistance = abs(currentPosition[0] - food[0]) + abs(currentPosition[1] - food[1])
        nearestFood = min(nearestFood, foodDistance)
    nearestFoodValue = 1000 / nearestFood
    features['nearestFood'] = nearestFoodValue
    ghostStates = state.getGhostStates()
    activeGhosts = 0
    for i in range(1, state.getNumAgents()):
        ghostDistance = abs(currentPosition[0] - ghosts[i-1][0]) + abs(currentPosition[1] - ghosts[i-1][1])
        nearestGhost = min(ghostDistance, nearestGhost)
    if (1 / nearestGhost) >= 0.5:
        nearestGhostValue = -99999
    else:
        nearestGhostValue = 1 / nearestGhost

    features['nearestGhost'] = nearestGhostValue
    
    capsules = state.getCapsules()
    closestCapsule = 999999
    for i, capsule in enumerate(capsules):
        closestcapsule = min(closestCapsule, abs(currentPosition[0] - capsule[0]) + abs(currentPosition[1] - capsule[1]))
    return features
